...
### cURL:
```bash
curl -X GET "https://seamless-sandbox-dev-cm.oriserve.com/callFunnelAnalytics?campaign_id=507f1f77bcf86cd799439013&start_date=2024-01-01T00:00:00Z" \
...
async function getCallFunnelAnalytics(filters) {
  const response = await axios.get(
    'https://seamless-sandbox-dev-cm.oriserve.com/callFunnelAnalytics',
    {
      params: filters,
      headers: {
  ...
  console.log(`\n⚠️ Highest Drop-off: ${drop_off_analysis.highest_drop_off_stage} (${drop_off_analysis.highest_drop_off_rate}%)`);
  console.log('\n💡 Recommendations:');
...
def visualize_funnel(campaign_id):
    url = "https://seamless-sandbox-dev-cm.oriserve.com/callFunnelAnalytics"
