...
### cURL:
```bash
curl -X POST "https://seamless-sandbox-dev-cm.oriserve.com/scheduleCrons" \
...
async function scheduleCronJob(config) {
  const response = await axios.post(
    'https://seamless-sandbox-dev-cm.oriserve.com/scheduleCrons',
    config,
    {
      headers: {
...
def schedule_cron_job(queue_name, action_name, cron_expression, timezone="Asia/Calcutta"):
    url = "https://seamless-sandbox-dev-cm.oriserve.com/scheduleCrons"
