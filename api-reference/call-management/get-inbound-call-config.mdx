...
### cURL:
```bash
curl -X POST "https://seamless-sandbox-dev-cm.oriserve.com/getAgentConfigAndUserDetailsForInboundCall" \
...
async function getInboundCallConfig(phoneNumber, callerNumber) {
  try {
    const response = await axios.post(
      'https://seamless-sandbox-dev-cm.oriserve.com/getAgentConfigAndUserDetailsForInboundCall',
      {
