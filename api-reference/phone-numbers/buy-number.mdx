...
### cURL:
```bash
curl -X POST "https://seamless-sandbox-dev-cm.oriserve.com/buy" \
...
async function buyPhoneNumber(number, alias) {
  try {
    const response = await axios.post(
      'https://seamless-sandbox-dev-cm.oriserve.com/buy',
      {
        number,
        config: {
          alias
...
def buy_phone_number(number, alias=None):
    url = "https://seamless-sandbox-dev-cm.oriserve.com/buy"
