...
### cURL:
```bash
curl -X DELETE "https://seamless-sandbox-dev-cm.oriserve.com/deleteUser?userId=507f1f77bcf86cd799439011" \
...
async function deleteUser(userId) {
  try {
    const response = await axios.delete(
      `https://seamless-sandbox-dev-cm.oriserve.com/deleteUser?userId=${userId}`,
      {
        headers: {
...
def delete_user(user_id):
    url = f"https://seamless-sandbox-dev-cm.oriserve.com/deleteUser?userId={user_id}"
