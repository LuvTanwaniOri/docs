...
**Endpoint:** `GET /callFunnelAnalytics`
...
### cURL:
```bash
curl -X GET "https://api.voicegenie.com/callFunnelAnalytics?campaign_id=507f1f77bcf86cd799439013&start_date=2024-01-01T00:00:00Z" \
...
async function getCallFunnelAnalytics(filters) {
  const response = await axios.get(
    'https://api.voicegenie.com/callFunnelAnalytics',
    {
      params: filters,
      headers: {
  ...
  console.log(`\n{% hint style="warning" %}
Highest Drop-off: ${drop_off_analysis.highest_drop_off_stage} (${drop_off_analysis.highest_drop_off_rate}%)`);
{% endhint %}
  console.log('\n💡 Recommendations:');
...
def visualize_funnel(campaign_id):
    url = "https://api.voicegenie.com/callFunnelAnalytics"
    ...
    if response.status_code == 200:
        ...
        print("📊 Funnel chart saved to funnel_analysis.png")
...
async function identifyBottlenecks(campaignId) {
  ...
  if (bottleneck) {
    console.log(`🎯 Major Bottleneck: ${bottleneck.stage_name}`);
