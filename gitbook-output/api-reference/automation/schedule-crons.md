...
### cURL:
```bash
curl -X POST "https://api.voicegenie.com/scheduleCrons" \
...
async function scheduleCronJob(config) {
  const response = await axios.post(
    'https://api.voicegenie.com/scheduleCrons',
    config,
    {
      headers: {
...
def schedule_cron_job(queue_name, action_name, cron_expression, timezone="Asia/Calcutta"):
    url = "https://api.voicegenie.com/scheduleCrons"
