...
### cURL:
```bash
curl -X DELETE "https://api.voicegenie.com/deleteUser?userId=507f1f77bcf86cd799439011" \
...
async function deleteUser(userId) {
  try {
    const response = await axios.delete(
      `https://api.voicegenie.com/deleteUser?userId=${userId}`,
      {
        headers: {
...
def delete_user(user_id):
    url = f"https://api.voicegenie.com/deleteUser?userId={user_id}"
...
### Effects:
1. ❌ **User cannot login** - `active` set to `false`
2. 🔒 **Account marked deleted** - `isDeleted` set to `true`
3. 🚫 **Filtered from GET requests** - Won't appear in `/getAllUsers`
4. 📁 **Data preserved** - Record remains in database for audit
...
**Important:** This is a soft delete operation. User data is preserved for audit purposes. 🔒
