...
```
Recording Ready → Webhook received → Verify signature
                                           ↓
                          ...
                          6. Update campaign analytics
                                           ↓
                          Return 200 OK → Process complete
```
...
### cURL Request:
```bash
curl -X POST https://seamless-sandbox-dev-cm.oriserve.com/callRecordingStatus \
...
**Recording processing is async and may take 30s-2min depending on duration.** ⏱️
